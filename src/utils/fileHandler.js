...
/**
 * Resolve explicit Markdown files declared under manifest `input.rtfm`.
 * The first entry is treated as the primary (non-supplemental) file, matching
 * the convention used by findReadmeFiles for its main + supplemental files.
 * @param {string} sourceDir - Directory the manifest paths are resolved against
 * @param {string[]} rtfmPaths - Paths (relative to sourceDir) to Markdown files
 * @returns {Array} Array of file objects with path and relative path info
 */
export async function resolveRtfmFiles(sourceDir, rtfmPaths) {
    return rtfmPaths.map((relPath, index) => {
        const fullPath = path.resolve(sourceDir, relPath);
        return {
            fullPath,
            fileName: path.basename(fullPath),
            directory: path.dirname(fullPath),
            isSupplemental: index !== 0,
        };
    });
}
...
export function buildOutputFilename(metadata, sourceDir) {
    try {
        ...
        const fallback = path.basename(sourceDir).toLowerCase().replace(/\s+/g, '-');
